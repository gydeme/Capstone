...
//}
//
//class PeakFinderOG(parquetPageCount:Boolean, parquetPagecountPath:String, dbHost:String, dbPort:Int, dbUsername:String, dbPassword:String,
...
//    .appName("Wikipedia activity detector")
//    .config("spark.cassandra.connection.host", dbHost)
//    .config("spark.cassandra.connection.port", dbPort.toString)
//    .config("spark.cassandra.auth.username", dbUsername)
//    .config("spark.cassandra.auth.password", dbPassword)
//    .config("neo4j.url", neo4jUrl) // Updated key
//    .config("neo4j.authentication.basic.username", neo4jUser)
//    .config("neo4j.authentication.basic.password", neo4jPass)
//    .config("neo4j.database", neo4jDb)
//    .getOrCreate()
//
//
...
////      .relationship(relsQuery, Map("nodelist" -> nodeList))
////      .loadGraph
...
////  }
//
//  //GDS implementation of extactActiveSubGraph
//  def extractActiveSubGraph(activeNodes: Dataset[Long], includeCategories: Boolean, graphName: String)(implicit session: SparkSession): Graph[String, Double] = {
//    import session.implicits._
//
//    // Collect active node IDs
//    val nodeList = activeNodes.collect().toSeq
//
//    // Define filters for GDS projection
//    val nodeFilter = if (includeCategories) {
//      s"Page WHERE id IN [${nodeList.mkString(",")}]"
//    } else {
//      s"Page WHERE id IN [${nodeList.mkString(",")}] AND NOT 'Category' IN labels(p)"
//    }
//
//    val relationshipFilter = if (includeCategories) {
//      s"ALL"
//    } else {
//      s"NOT 'Category' IN labels(startNode(r)) AND NOT 'Category' IN labels(endNode(r))"
//    }
//
//    // Create a GDS graph projection
//    session.sqlContext.read.format("org.neo4j.spark.DataSource")
//      .option("gds.graphName", graphName)
//      .option("gds.nodeProjection", nodeFilter)
//      .option("gds.relationshipProjection", relationshipFilter)
//      .load()
//
//    val verticesDF = session.sqlContext.read.format("org.neo4j.spark.DataSource")
//      .option("query",
//        s"""
//      CALL gds.graph.streamNodeProperties('${graphName}', ['id', 'title'])
//      YIELD nodeId, properties
//      RETURN nodeId AS id, properties['title'] AS title
//    """).load()
//    verticesDF.printSchema()
//    verticesDF.show(5)
//
//    val edgesDF = session.sqlContext.read.format("org.neo4j.spark.DataSource")
//      .option("query",
//        s"""
//      CALL gds.graph.streamRelationship('${graphName}')
//      YIELD sourceNodeId, targetNodeId, properties
//      RETURN sourceNodeId AS source, targetNodeId AS target, properties['weight'] AS weight
//    """).load()
//    edgesDF.printSchema()
//    edgesDF.show(5)
//
//    // Read back projected graph data
//    val vertices = session.sqlContext.read.format("org.neo4j.spark.DataSource")
//      .option("query",
//        s"""
//      CALL gds.graph.streamNodeProperties('${graphName}', ['id', 'title'])
//      YIELD nodeId, properties
...
//    """).load().as[(Long, String)]
//
//    val edges = session.sqlContext.read.format("org.neo4j.spark.DataSource")
//      .option("query",
//        s"""
//      CALL gds.graph.streamRelationship('${graphName}')
//      YIELD sourceNodeId, targetNodeId, properties
//      RETURN sourceNodeId AS source, targetNodeId AS target, properties['weight'] AS weight
//    """).load().as[(Long, Long, Double)]
//
//    // Convert vertices and edges to RDDs
//    val vertexRDD = vertices.rdd.map { case (id, title) => (id, xml.Utility.escape(title)) }
//    val edgeRDD = edges.rdd.map { case (source, target, weight) => Edge(source, target, weight) }
//
...
//    val pf = new PeakFinderOG(cfgBase.parquetPagecounts(), cfgBase.parquetPagecountsPath(),
...
//
//    val graphName = "activePagesGraph"
//    val activePagesGraph = GraphUtils.toUndirected(
//      pf.extractActiveSubGraph(activePages, includeCategories, graphName)(pf.session)
